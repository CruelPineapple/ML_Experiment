...
class Node(namedtuple("Node","children type content feature label")):
    # 节点内容:孩子节点, 分类特征的数值, 节点内容, 节点分类特征,标签
    def __repr__(self):
        return str(tuple(self))


class DecisionTree:
    def __init__(self, method = "info_gain_ratio"):  #缺省采用C4.5即信息增益比
        self.tree = None
        self.method = method  #采用信息增益还是信息增益比
    ...
    def _information_gain(self, X_train, y_train, feature):
        #  计算信息增益
        return self._experience_entropy(y_train) - self._conditinal_entropy(X_train, y_train, feature)
